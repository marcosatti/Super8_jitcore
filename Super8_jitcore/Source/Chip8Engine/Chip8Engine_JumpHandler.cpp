...
=======
			_snprintf_s(buffer, 1000, "Conditional Jump[%d] updated! Value %d written to 0x%.8X (in cache[%d]).", i, relative, (uint32_t)cond_jump_list->get_ptr(i)->x86_address_jump_value, cache->findCacheIndexCurrent());
>>>>>>> block_test_perf
			logMessage(LOGLEVEL::L_INFO, buffer);
#endif
...
void Chip8Engine_JumpHandler::DEBUG_printJumpList()
{
	for (int32_t i = 0; i < (int32_t)jump_list->size(); i++) {
		char buffer[1000];
		_snprintf_s(buffer, 1000, "Jump[%d]: c8_address_to = 0x%.4X, x86_address_to = 0x%.8X.", i, jump_list->get_ptr(i)->c8_address_to, (uint32_t)jump_list->get_ptr(i)->x86_address_to);
