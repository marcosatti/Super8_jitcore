...
namespace Chip8Globals {
	namespace C8_STATE {
		...
		void C8_clearGFXMem();
		void C8_clearRegV();
		void C8_incrementPC(uint8_t bytes = 2); // 2 Bytes by default
		void C8_deallocate();
		uint8_t C8_getPCByteAlignmentOffset(uint16_t c8_pc); // PC always starts at 0x0200, and each opcode is always 2 bytes long. So if there is a jump to eg: 0x0211, then it has an alignment offset of 1 (alignment of 0 would be 0x0210 or 0x0212).
